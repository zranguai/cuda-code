/**************************************************************************************
 *  file name   : reduceUnrolling.cu
 *  brief       : 展开循环
 *  ref         : https://face2ai.com/CUDA-F-3-5-%E5%B1%95%E5%BC%80%E5%BE%AA%E7%8E%AF/
***************************************************************************************/
...
__global__ void reduceUnroll2(int * g_idata,int * g_odata,unsigned int n)
{
	...
	if (tid >= n) return;
	//convert global data pointer to the
	int *idata = g_idata + blockIdx.x*blockDim.x*2;  // 间隔两个block
	if(idx+blockDim.x<n)
	{
		g_idata[idx]+=g_idata[idx+blockDim.x];  // // 相邻的block进行向量加法
	...
	for (int stride = blockDim.x/2; stride>0 ; stride >>=1)
	{
		if (tid <stride)
		{
			idata[tid] += idata[tid + stride];  // 交错配对进行规约
		}
		//synchronize within block
		__syncthreads();
	}
...
__global__ void reduceUnrollWarp8(int * g_idata,int * g_odata,unsigned int n)
{
	...
	if(tid<32)
	{
		volatile int *vmem = idata;  // volatile int类型变量控制变量结果写回到全局内存中，而不是缓存或共享内存中
        // tid+32与tid+16不会造成内存竞争，因为一个线程束，执行的进度是完全同步的
        // 当执行tid+32时，这32个线程都在执行这步
...
// 函数模板规约 展开最后的128个，256个，512个，1024个
...
int main(int argc,char** argv)
{
	...
	gpu_sum = 0;
    // 这里相比较之前 需要grid.x/2 因为间隔两个block
	...
	cudaMemcpy(odata_host, odata_dev, grid.x * sizeof(int), cudaMemcpyDeviceToHost);
	gpu_sum = 0;
    // 这里相比较之前 需要grid.x/4 因为间隔四个block
