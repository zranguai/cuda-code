...
### 线程束洗牌指令
> 作用在线程束内，允许两个线程间相互访问对方的寄存器。使得不需要通过访问内存进行线程间的数据交换，具有非常高的性能

[线程束洗牌指令](https://face2ai.com/CUDA-F-5-6-%E7%BA%BF%E7%A8%8B%E6%9D%9F%E6%B4%97%E7%89%8C%E6%8C%87%E4%BB%A4/)
...
物理层面上，一个SM上的所有的正在执行的线程块共同使用物理的共享内存。共享内存也成为活跃线程块的限制。共享内存越大，或者块使用的共享内存越小，那么线程块级别的并行度就越高。每个SM都有一定数量的由线程块分配的共享内存，共享内存是片上内存，跟主存相比，速度要快很多，也即是延迟低，带宽高。其类似于一级缓存，但是可以被编程。
...
怎么理解宽度呢，我们假设我们这有三十二个水桶，每个水桶当做一个存储体，桶的口大小是固定的，假设我们用桶装西瓜，每个桶的口最多同时能拿出四个西瓜，能拿出四个西瓜，宽度就是4，能拿出八个就是八，这就是宽度的通俗解释。共享内存的存储体的访问模式就是： 从0开始， 如果是四字节， 0-3装到0号桶，4-7装到1号桶，... 32-35又装到0号桶。

存储体的索引可按下面公式进行计算(/4是存储体宽度  /存储体数(32)是线程束里的线程数目)
```
    存储体索引 = (字节地址 / 4 / 存储体数)%存储体数
```

### 解决共享内存存储体冲突--内存填充
共享内存存储体冲突会严重影响共享内存的效率，那么但我们遇到严重冲突的情况下，可以使用填充的办法让数据错位，来降低冲突。
假如我们当前的存储体内的数据罗列如下：

![存储体冲突01](./images/存储体冲突01.png)

当我们的线程束访问bank0中的不同数据的时候会发生一个5线程的冲突，这时候我们假如我们分配内存时候的声明是下面时，这个时候会得到上面的这种内存分布布局：
```
...
```
就会产生如下效果，在编程的时候加入一行填充物

![存储体冲突02](./images/存储体冲突02.png)

然后编译器会将这个二维数组重新分配到存储体，因为存储体一共就4个，我们一行有五个元素，所以有一个元素进入存储体的下一行，这样所以的元素都错开了，就不会出现冲突了。

![存储体冲突02](./images/存储体冲突03.png)

注意：共享内存声明时，就决定了每个地址所在的存储体，想要调整每个地址对应的存储体，就要扩大声明的共享内存的大小，至于扩大多少，就要根据我们前面的公式好好计算了。

### 共享内存访问模式配置
可以通过以下语句，查询内存存储体宽度是4字节还是8字节：
```
...
### 共享内存的数据分布(案例分析)
> 参考代码: shared_memory/shared_memory_read_data

这里主要研究共享内存的数据分布，以及如何使用动态的共享内存，以及使用填充来避免冲突
[共享内存的数据分布](https://face2ai.com/CUDA-F-5-2-%E5%85%B1%E4%BA%AB%E5%86%85%E5%AD%98%E7%9A%84%E6%95%B0%E6%8D%AE%E5%B8%83%E5%B1%80/)
...
### 显示障碍
CUDA中，障碍点设置在核函数中，注意这个指令只能在**核函数**中调用，并只对**同一线程块(SM)内线程**有效。
```
void __syncthreads();
```
...
### Volatile修饰符
> 案例在03_cuda_memory_model/thread_warp/thread_warp_demo03/reduceUnrolling.cu line:199
...
## 常量内存
> 用于只读数据和线程束统一访问某一个数据，对内核代码只读，对于主机可读可写。常量内存的最优访问模式是线程束所有线程访问同一个位置。生命周期与应用程序相同。
...
### 矩阵转置
> 代码在memory_access/transform_matrix2D
矩阵转置就是交换矩阵的坐标，这里研究二维矩阵，转置的结果如下
![矩阵转置01](./images/矩阵转置01.png)

我们所有的数据，结构体也好，数组也好，多维数组也好，所有的数据，在内存硬件层面都是一维排布的，所以我们这里也是使用一维的数组作为输入输出，那么从真实的角度看内存中的数据就是下面这样的：
![矩阵转置02](./images/矩阵转置02.png)
...
交叉访问是使得内存访问变差的罪魁。但是作为矩阵转置本身，这个是无法避免的。但是在这种无法避免的交叉访问
