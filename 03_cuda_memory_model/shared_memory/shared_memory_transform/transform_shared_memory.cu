/**************************************************************************************
 *  file name   : transform_shared_memory.cu
 *  brief       : 使用共享内存，+padding并且展开优化转置操作
 *  ref         : https://face2ai.com/CUDA-F-5-4-%E5%90%88%E5%B9%B6%E7%9A%84%E5%85%A8%E5%B1%80%E5%86%85%E5%AD%98%E8%AE%BF%E9%97%AE/
***************************************************************************************/
...
// 使用共享内存的矩阵转置  先使用二维共享内存缓存原始矩阵数据，然后从共享内存中读取一列存储到全局内存中
...
// 读取共享内存的列的时候会有冲突 加padding操作建设冲突
...
// 使用展开的矩阵转置(以下同时操作两行矩阵进行转置)  相当于一个块干两个块的事情
