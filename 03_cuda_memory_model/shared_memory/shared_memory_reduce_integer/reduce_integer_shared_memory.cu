/**************************************************************************************
 *  file name   : reduce_integer_shared_memory.cu
 *  brief       : 使用共享内存优化并行规约
 *  ref         : https://face2ai.com/CUDA-F-5-3-%E5%87%8F%E5%B0%91%E5%85%A8%E5%B1%80%E5%86%85%E5%AD%98%E8%AE%BF%E9%97%AE/
***************************************************************************************/
...
// 使用共享内存进行优化 先存入共享内存然后再从共享内存存入全局内存
__global__ void reduceSmem(int * g_idata,int * g_odata,unsigned int n)
{
	//set thread ID
    __shared__ int smem[DIM];  // 共享内存
	unsigned int tid = threadIdx.x;
	//unsigned int idx = blockDim.x*blockIdx.x+threadIdx.x;
	//boundary check
	if (tid >= n) return;
