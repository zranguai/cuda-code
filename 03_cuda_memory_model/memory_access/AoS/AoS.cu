/**************************************************************************************
 *  file name   : AoS.cu
 *  brief       : 数组结构体(内存利用率低)测试代码
 * 开启一级缓存进行编译: nvcc -O3 -arch=sm_35 -Xptxas -dlcm=ca -I ./include/ AoS.cu -o  AoS
 * 禁用一级缓存进行编译: nvcc -O3 -arch=sm_35 -Xptxas -dlcm=cg -I ./include/ AoS.cu -o  AoS
***************************************************************************************/
